...
- supports multiple backends (like djvu, pymupdf)
- continuous scrolling
- supports viewing and scrolling 2 or more pages side-by-side where
  the view can be changed by simply using a numeric prefix
...
** Issues
By default the cursor should not get displayed, excpet when using
evil-mode where it appears to not be trivial to hide the cursor (see
